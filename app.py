...
class QAConfig:
    ...
    PAGE_TITLE = "AI QA Automation Dashboard"
    PAGE_ICON = "🤖"
...
class QAAnalyzer:
    ...
    def analyze_audio(self, file_path):
        ...
        prompt = """
        Act as a highly critical Senior QA Auditor for a Medical Billing and Brace company. 
        Your goal is to find every single mistake the agent made. Do not be lenient.

        ### SCORING RUBRIC (Total 100 Points):
        1. Greeting & Introduction (10 pts)
        2. HIPAA & Identity Verification (30 pts)
        3. Medical Data Extraction (30 pts)
        4. Soft Skills & Empathy (20 pts)
        5. Closing & Next Steps (10 pts)

        ### CRITICAL FAILURES (Automatic Score below 50):
        - Coercive or pushy behavior.
        - Failure to verify Medicare ID.

        Task 1: Data Extraction
        Extract: Agent_Name, Patient_Name, DOB, Address, Phone_Number, Medicare_ID, Brace_Size, Height, Weight, Pain_Level, Doctor_Name, Last_Visit_Date, Previous_Treatments.

        Task 2: Evaluation
        - Score: SINGLE NUMBER only.
        - Weaknesses: Precise timestamps (mm:ss).
...
class UIHandler:
    ...
    def render_header():
        st.markdown("""
            <div class="main-header">
                ...
                <p>Powered by Outsourcing Skill - Automated Quality Assurance</p>
            </div>
        """, unsafe_allow_html=True)
    ...
    def render_results(result):
        ...
        tab1, tab2 = st.tabs(["🌟 Strengths", "⚠️ Weaknesses & Critical Issues"])
        with tab1:
...
def main():
    ...
    if uploaded_file:
        ...
        if st.sidebar.button("🚀 Analyze Call Now"):
            with st.spinner('🤖 AI Auditor is analyzing...'):
